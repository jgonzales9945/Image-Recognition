...
int main ( /*int argc, char** argv*/ ) {
	...
	do {
		...
		string arg;
		cin >> arg;
		src = cv::imread(arg, CV_LOAD_IMAGE_COLOR);
		if(src.empty())	{
		...
		do {
			...
			if(input == '1' || input == '2' || input == '3' || input == '4' || input == '5') writeTo(mod, name);
...
Mat morph(Mat& img) {
	...
	namedWindow("morphed", CV_WINDOW_AUTOSIZE);
	imshow("morphed", dst);
	return dst;
}
...
Mat cannyThreshold(Mat& img, int e) {
	...
	do {
		...
		namedWindow( "Canny Edge Detect", CV_WINDOW_AUTOSIZE);
		...
		if(e == 1) {
			...
			for(int i = 0; i < contours.size(); i++) { 
				if(contours[i].size() >= sizeMin && contours[i].size()  < sizeMax) minEllipse[i] = minAreaRect(Mat(contours[i])); 
			}
			...
			namedWindow( "Ellipse fit", CV_WINDOW_AUTOSIZE);
...
Mat blue(Mat& img) {
	...
	namedWindow("Blue removal", CV_WINDOW_AUTOSIZE);
